...
for idx, (text, color, *args) in enumerate(button_configs):
    ...
    command = open_settings_screen if text == "Settings" else (open_progress_screen if text == "Progress" else lambda color=color, start_session=start_session: open_screen(color, start_session))
