...
class ColorState:
    ...
    def _is_valid_color(self, color):
        try:
            return isinstance(color, str) and (
                ...
                color in ['white', 'black', 'red', 'blue', 'green']
            )
        except:
            return False
    ...
    def update_colors(self, bg, fg):
        changed = False
        if self._is_valid_color(bg):
            self._bg_color = bg
            changed = True
        if self._is_valid_color(fg):
            self._fg_color = fg
            changed = True
        if changed:
...
def open_start_session_screen():
    ...
    # Color display frame
    color_display = tk.Frame(screen, bg="#1d5899", bd=2, relief="solid")
    color_display.place(relx=0.5, rely=0.3, anchor="center")

    # Labels for displaying colors
    bg_color_label = tk.Label(
       color_display,
       text=f"Background: {color_state.bg_color}",
       bg="#1d5899",
       fg="#a0e4cb",
       font=("Arial", 10)
    )
    bg_color_label.pack(padx=5, pady=2)

    fg_color_label = tk.Label(
        color_display,
        text=f"Foreground: {color_state.fg_color}",
        bg="#1d5899",
        fg="#a0e4cb",
        font=("Arial", 10)
    )
    fg_color_label.pack(padx=5, pady=2)

    # Update function for color labels
    def update_color_display():
        if color_display.winfo_exists():
            bg_color_label.config(text=f"Background: {color_state.bg_color}")
            fg_color_label.config(text=f"Foreground: {color_state.fg_color}")
            screen.after(100, update_color_display)

    # Start updating color display
    ...
    def update_document_colors():
        if document_text.winfo_exists():
    ...
    def update_display_optimization():
        ...
        if data_collector.session_active and document_text.winfo_exists():
            try:
                ...
                if states:
                    # Get trends and optimize colors
                    trends = brain_analyzer.get_state_trends(states)
                    brain_analyzer.optimize_colors(states, trends)

                # Schedule next update
                screen.after(100, update_display_optimization)
            except tk.TclError:
                pass
    ...
    back_button = tk.Button(
        screen,
        text="Back",
        command=lambda: show_main_screen(screen),
        bg="#a0e4cb",
        font=("Arial", 10)
    )
    back_button.place(relx=0.5, rely=0.9, anchor="center")
